...
log = open("last_card_play_log.txt", "w")
log.write(f"logging initialized\n executing file : \n{__file__}\n on : {datetime.datetime.now()}\n")

def CardGenerate(cl): #fungsi menghasilkan semua 52 kartu main secara otomatis
    print("tips : menggunakan opsi 2 (alfabet) lebih baik untuk testing dan logging")
    ...
    if style == "1": # memilih penggunaan output yang akan digunakan untuk simbol kartu
        log.write(f"using style option 1 (Unicode)\nlogging discontinued because unicode is unsupported in text files\n")
        default_symbols = {'♦️' : 0.1, '♣️' : 0.2, '♥️' : 0.3, '♠️' : 0.4} #list simbol limbol kartu (logo/Unicode)
    elif style == "2":
        log.write(f"using style option 2 (Alphabet)\n")
...
def GiveCard(): #method tuntuk membagikan kartu pada masing-masing pemain seacra acak
    ...
    choose_player = random.choice(list_players)
    if len(list(choose_player)) != 13:
        ...
        try:
            ...
            if style != "1":
                ...
                log.write(f"{choose_player} got: {choose_card}\n")
        except:
            print("", end=" ")
...
log.write(f"\nrunning CardGenerate() function\n")
cards_list, style = CardGenerate(cl)

if style != "1":
    for evrycards in cards_list:
        log.write(f"{evrycards} : {cl[evrycards]}\n")


#ShowDealerCards()
log.write(f"\nrunning GiveCard() function\n")
while len(cards_list) != 0:
    GiveCard()

print(f"\n{bwhite}{Black}generated card : {res}")
test=  ShowAllPlayerCards()
if style != "1":
    log.write(f"\ndistributed cards :")
    log.write((f"\nPLAYER 1 : {list(p1)} \nPLAYER 2 : {list(p2)} \nPLAYER 3 : {list(p3)} \nPLAYER 4 : {list(p4)}"))
